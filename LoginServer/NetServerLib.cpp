...
void NetServer::SetHeartBeat(uint64_t sessionID)
{
	if (AcquireSession(sessionID))
	{
...
void NetServer::IO_Cancel(Session* curSession)
{
	...
	//CancelIoEx((HANDLE)curSession->_Socket, &curSession->_RecvOL);
}
...
bool NetServer::SendPacket(uint64_t sessionID, NetPacket* packet)
{
	...
	if (!AcquireSession(sessionID))
	{
		...
		//_LOG(LOG_LEVEL_ERROR, L"AcquireSesion Packet Decremetnt(%d->%d)", packet->m_RefCount, packet->m_RefCount - 1);
...
bool NetServer::SendPost(uint64_t  sessionID)
{
	...
	if (!AcquireSession(sessionID))
	{
		...
		// 성공한 후, SendFlag에 진입을 실패하면  루프밖에서 IOCount를 감소시킨다.
		//-------------------------------------------------
		return false;
	}
	do
